...
# The seven requested brands use screenshot-reference artwork.
REFERENCE_SPRITE_LOGOS = {'CHUBB','GENERALI','TAKAFUL IKHLAS','LONPAC','RHB','TAKAFUL MALAYSIA'}
REFERENCE_SPRITE = 'insurance-screenshot-six-card-q50.jpg'
REFERENCE_AIG = 'insurance-screenshot-seven-card-q50.jpg'
...
def clean_logo(src_name, dst_name):
    ...
    while q:
        ...
        if a==0 or colour_distance((r,g,b),bg)>fuzz: continue
        px[x,y]=(0,0,0,0)
    ...
    for y in range(h):
        for x in range(w):
            r,g,b,a=px[x,y]
            if a==0: continue
            mx,mn=max(r,g,b),min(r,g,b); sat=0 if mx==0 else (mx-mn)/mx
            lum=.299*r+.587*g+.114*b
            if sat<.18 and lum<125: px[x,y]=(255,255,255,a)
    bbox=im.getbbox()
...
for brand_name,src in LOGOS:
    dst=f'{Path(src).stem}-clean.png'; clean_logo(src,dst); clean_files.append(dst)
...
for (name,_),clean_file in zip(LOGOS,clean_files):
    ...
    if name=='AIG':
        src_file=REFERENCE_AIG; ref_class=' reference-logo reference-aig'
    elif name in REFERENCE_SPRITE_LOGOS:
        src_file=REFERENCE_SPRITE; ref_class=' reference-logo reference-sprite'
    else:
        src_file=clean_file; ref_class=''
    cards.append(f'<div class="insurance-panel-card" title="{name}"><img class="insurance-logo insurance-logo-{slug}{ref_class}" src="{src_file}" alt="{name} insurance logo" loading="lazy"></div>')
...
if count!=1: raise SystemExit('insurance-panels section not found')
...
/* Exact screenshot-reference crops: no cleanup/recoloring is applied. */
.insurance-panel-card .reference-aig{width:106px;max-width:106px;height:55px;object-fit:contain}
.insurance-panel-card .reference-sprite{width:218px;max-width:none;height:100px;object-fit:none;image-rendering:auto}
.insurance-panel-card .insurance-logo-chubb.reference-sprite{object-position:center 36px}
.insurance-panel-card .insurance-logo-generali.reference-sprite{object-position:center -36px}
.insurance-panel-card .insurance-logo-lonpac.reference-sprite{object-position:center -151px}
.insurance-panel-card .insurance-logo-rhb.reference-sprite{object-position:center -240px}
.insurance-panel-card .insurance-logo-takaful-ikhlas.reference-sprite{object-position:center -320px}
.insurance-panel-card .insurance-logo-takaful-malaysia.reference-sprite{object-position:center -438px}
