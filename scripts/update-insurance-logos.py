...
# Use the valid JPEG screenshot sprite for the six logos whose screenshot
# crops were prepared previously. AIG uses its local cleaned asset because
# the earlier seven-logo sprite was actually PNG bytes saved with a .jpg
# extension, which caused the browser corruption.
REFERENCE_SPRITE_LOGOS = {
    'CHUBB', 'GENERALI', 'TAKAFUL IKHLAS', 'LONPAC', 'RHB', 'TAKAFUL MALAYSIA'
}
...
for (name, _), clean_file in zip(LOGOS, clean_files):
    ...
    src_file = REFERENCE_SPRITE if name in REFERENCE_SPRITE_LOGOS else clean_file
    ref_class = ' reference-logo' if name in REFERENCE_SPRITE_LOGOS else ''
    cards.append(
...
/* Six screenshot-reference logos use the valid JPEG sprite. */
